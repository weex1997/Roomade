        ...
        db.collection("users").whereField("showProfile", isEqualTo: true).getDocuments { doucments, error in
            ...
            if let error = error {
                ...
                for document in documents {
                    ...
                    let user = User(name: name ?? "", city: city ?? "", budget: budget ?? "", available: available ?? "", Gender: Gender ?? "", phoneNumber: phoneNumber ?? "", about: about ?? "" , interest: interest ?? "", disrticts: disrticts ?? "", PeriodOfStay: PeriodOfStay ?? "", RoomType: RoomType ?? "", Conditions: Conditions ?? "", showProfile: showProfile ?? false ,  date : date ?? Date()) 
